...
> ETL-Toolbox - Open source tool by AetherCodeHQ

`cli` `data` `etl` `golang` `pipeline`

## What is it?

**Etl Toolbox** is a data tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/ETL-Toolbox.git
cd ETL-Toolbox
go build -o ETL-Toolbox .
```
...
```bash
# Basic usage
./ETL-Toolbox --path ./target

# With options
./ETL-Toolbox --path ./target --format json --output report.json

# Verbose mode
./ETL-Toolbox --path ./target --verbose
```
...
### Basic scan
```bash
./ETL-Toolbox --path ./my-project
```

### JSON report
```bash
./ETL-Toolbox --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Etl Toolbox
  run: ./ETL-Toolbox --path . --format json --output report.json
```
